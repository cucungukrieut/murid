...
<br/>
 <h2 align="left" style="color:Darkred;">(LASER) Paper Total Cost in Letter</h2>
...
FROM printingreports WHERE PaperSize = 'Letter' AND printertype = 'Laser' GROUP BY YEAR(daily), MONTH(daily), DAY(daily) ORDER BY DAY(daily) Asc");
...
while($rs1 = mysql_fetch_array($sql)){
  ...
  FROM printingreports WHERE PaperSize = 'Letter' AND printertype = 'Laser'");
