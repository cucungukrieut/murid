...
<br/>
 <h2 align="left" style="color:Darkred;">(INKJET) Paper Total Cost in Legal</h2>
...
FROM printingreports WHERE PaperSize = 'Legal' AND printertype = 'Inkjet' GROUP BY YEAR(daily), MONTH(daily), DAY(daily) ORDER BY DAY(daily) Asc");
...
while($rs1 = mysql_fetch_array($sql)){
  ...
  FROM printingreports WHERE PaperSize = 'Legal' AND printertype = 'Inkjet'");
  $totallegal = mysql_fetch_array($total);
  echo "<b>Total Legal Size : </b>".$totallegal['total'];  
?>
